...
import 'amenities_recreation.dart';
import '../Ameneties_Meeting_Room/amenities_list_screen.dart';

/// Amenities screen - Social/Utility tab (Family hall, BBQ area, Meeting room, Kids play area).
class AmenitiesSocialUtilityScreen extends StatelessWidget {
  const AmenitiesSocialUtilityScreen({super.key});

  static const List<_AmenityItem> _items = [
    _AmenityItem(
      title: 'Family hall',
      description: 'Spacious area for family gatherings and celebrations.',
      iconPath: 'assets/services/family.png',
      showCrown: false,
    ),
    _AmenityItem(
      title: 'BBQ area',
      description: 'Outdoor grilling space for fun meals with friends and family.',
      iconPath: 'assets/services/BBQ.png',
      showCrown: false,
    ),
    _AmenityItem(
      title: 'Meeting room',
      description: 'Professional space for meetings and discussions.',
      iconPath: 'assets/services/meeting.png',
      showCrown: true,
    ),
    _AmenityItem(
      title: 'Kids play area',
      description: 'Safe and fun zone designed for children to enjoy.',
      iconPath: 'assets/services/kids.png',
      showCrown: false,
    ),
  ];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => Navigator.of(context).pop(1), // Return chip index 1 (Social/Utility)
        ),
        centerTitle: true,
        title: const Text(
          'Amenities',
...
class _ChipsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const selectedIndex = 1; // Social/Utility
          ...
          return GestureDetector(
            onTap: () {
              if (index == 0) {
                ...
                // Open Recreation; forward pop value to Facilities so active chip stays in sync.
                Navigator.of(context)
                  ...
                  MaterialPageRoute<int>(
                    ...
                    .then((value) {
                  // Forward chip index to Facilities so it can sync. Don't forward 2 (Recreation) so back from Recreation keeps us here.
                  if (value != null && value != 2) Navigator.of(context).pop(value);
                });
              } else if (index == 3) {
                Navigator.of(context).pop(3);
              }
              // index == 1 (Social/Utility): already on this screen
            },
            child: Container(
...
/// Fixed card height so all cards (including Kids play area) are the same height.
const double _cardHeight = 180.0;
...
class _AmenityCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        if (item.title.toLowerCase() == 'meeting room') {
              ...
              ClipRRect(
                ...
                child: Image.asset(
                  ...
                  errorBuilder: (_, __, ___) => Container(
                    ...
                    child: const Icon(Icons.groups_2_outlined, color: AppTheme.textSecondary),
                  ),
                ),
              ),
              if (item.showCrown) ...[
